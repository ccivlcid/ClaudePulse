...
import { getSessionFilePath, getServerLogPath, getPulseDir, deleteSession, deleteAllData } from '../data/writer.js';
...
app.get('/api/sessions/active', (c) => {
  ...
  if (project) {
    ...
    if (sessions.length === 0) return c.json({ error: 'No active session for this project' }, 404);
