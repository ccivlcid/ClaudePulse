...
| `/claude-pulse:errors`    | 세션 내 모든 에러 (도구 실패 + 서버 에러) |
| `/claude-pulse:heatmap`   | 파일 접근 빈도 순위                 |
| `/claude-pulse:agents`    | 활성 에이전트 상태                  |
| `/claude-pulse:tokens`    | 토큰 사용량 추정                   |
| `/claude-pulse:timeline`  | 세션 타임라인 요약                  |
| `/claude-pulse:health`    | Dev 서버 상태 종합 진단              |
| `/claude-pulse:reset`     | 세션 데이터 또는 전체 데이터 초기화        |
