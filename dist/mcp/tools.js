...
export function pulseSessionStats(params) {
...
export function pulseFileHeatmap(params) {
    const sid = resolveSessionId(params.sessionId);
    if (!sid)
        return error('활성 세션이 없습니다.');
    const heatmap = getFileHeatmap(sid).slice(0, params.top ?? 10);
    if (heatmap.length === 0)
        return text('파일 접근 기록이 없습니다.');
    const lines = heatmap.map((f, i) => `${i + 1}. ${f.filePath}  (Read:${f.readCount} Edit:${f.editCount} Write:${f.writeCount} = ${f.total})`);
    return text(`── File Heatmap ──\n${lines.join('\n')}`);
}
export function pulseAgentStatus(params) {
    const sid = resolveSessionId(params.sessionId);
    if (!sid)
        return error('활성 세션이 없습니다.');
    const agents = getAgentStatus(sid);
    if (agents.length === 0)
        return text('에이전트 활동이 없습니다.');
...
export function pulseTimeline(params) {
    const sid = resolveSessionId(params.sessionId);
    if (!sid)
        return error('활성 세션이 없습니다.');
    const timeline = getTimeline(sid);
    if (timeline.length === 0)
        return text('이벤트가 없습니다.');
...
function readServerLogs(sessionId) {
...
export function pulseServerLogs(params) {
    const sid = resolveSessionId(params.sessionId);
    if (!sid)
        return error('활성 세션이 없습니다.');
...
export function pulseServerErrors(params) {
    const sid = resolveSessionId(params.sessionId);
    if (!sid)
        return error('활성 세션이 없습니다.');
    const status = getServerStatus();
...
export function pulseServerHealth(params) {
    const sid = resolveSessionId(params.sessionId);
    if (!sid)
        return error('활성 세션이 없습니다.');
    ...
    const detectedPort = managed?.port
        ?? logs.reduceRight((found, l) => found ?? extractPort(l.text), null);
...
export function pulseStartServer(params) {
...
export function pulseStopServer() {
