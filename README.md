...
| **File Heatmap** | 가장 많이 접근한 파일을 트리맵으로 시각화 |
| **Agent Tracker** | 활성 에이전트와 활동량 추적 |
| **Cost Estimate** | 세션별 추정 비용 (도구 호출 기반) |
| **Session Timeline** | 세션 전체를 시간축으로 조망 |
| **Error Panel** | 모든 에러를 통합 표시 |
| **Server Monitor** | Dev 서버 로그 실시간 표시 |
| **Project Comparison** | 프로젝트/세션 간 통계 비교 |
...
JSONL 한 줄 예시:
```json
{"id":"a1b2","ts":"2026-03-30T14:32:01Z","type":"tool-start","tool":"Read","file":"src/auth.ts","session":"abc-123"}
```
...
| `pulse_cost_estimate` | 비용 추정 | "지금까지 얼마나 쓴 것 같아?" |
...
비용 추정은 도구 호출의 input 크기와 도구별 평균 output 토큰 테이블을 기반으로 합니다.

Hook에서는 시스템 프롬프트, 대화 히스토리 등의 토큰을 볼 수 없기 때문에, **실제 비용의 30~60% 수준만 추정 가능**합니다. 추정치는 세션 간/프로젝트 간 **상대 비교**에 유용하며, 절대 금액은 참고용입니다.
